...
export default function BookDetailsLocalScreen() {
  ...
  // Extrair ID do livro do params e buscar dados completos
  ...
  const handleSubmitReview = async () => {
    ...
    if (isEditing && editingReviewId) {
      ...
      const bookImage = livro?.capaUri || params.image as string || "https://via.placeholder.com/150";
      result = await createReview(bookId, livro?.titulo || "Livro Cadastrado", livro?.autor || "Desconhecido", bookImage, rating, comment);
    }
  ...
  const handleFavoriteToggle = async () => {
    ...
    if (favorite) {
      ...
      const result = await addFavorite(bookId, livro?.titulo || "Livro Cadastrado", livro?.autor || "Desconhecido", bookImage);
      if (result.success) {
        Toast.show({
          type: 'success',
          text1: 'Favoritado',
