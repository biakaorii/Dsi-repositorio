  ...
  const renderLivroItem = (livro: Livro) => {
    ...
    return (
      <View
        ...
        style={[
          styles.bookProgress,
          categoria === "lendo" && livroCompleto && styles.completedReadingBook,
          categoria === "lidos" && styles.completedBook,
          categoria === "queroLer" && (livro.salvo ? styles.wishlistBook : styles.wishlistBookUnsaved),
        ]}
      >
        ...
        <View style={styles.summaryStats}>
          {(["lidos", "lendo", "queroLer"] as Categoria[]).map((cat) => (
            <View key={cat} style={styles.summaryCard}>
              <Text style={styles.summaryNumber}>{livros[cat].length}</Text>
              <Text style={styles.summaryLabel}>{getCategoryLabel(cat)}</Text>
            </View>
          ))}
        </View>
      </View>
          ...
          >
            <Text
              style={[
                ...
                categoriaAtiva === cat && styles.categoryButtonTextActive,
              ]}
            >
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 80 }}>
        <View style={styles.section}>
          {livros[categoriaAtiva].length === 0 ? (
            <Text style={styles.emptyText}>{getEmptyMessage(categoriaAtiva)}</Text>
          ) : (
